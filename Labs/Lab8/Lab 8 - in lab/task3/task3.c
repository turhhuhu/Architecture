...
void printReloc (state *s)
{
    ...
    for(int i = 0; i < section_headers_table_size; i++)
    {
        ...
        if(header_type == SHT_REL)
        {
            ...
            Elf32_Rel *rel_start = (Elf32_Rel *)(s->map_start + rel_offset);
            int rel_size = rel_header -> sh_size;
            int number_of_rel = rel_size / sizeof(Elf32_Rel);

            // first linking (symbol table)
            int dynsym_header_index = rel_header -> sh_link;
            Elf32_Shdr *dynsym_header = &(shdr_start[dynsym_header_index]);
            int dynsym_table_offset = dynsym_header -> sh_offset;
            Elf32_Sym *dynsym_start = (Elf32_Sym *)(s->map_start + dynsym_table_offset);

            // second linking (str table for the symbol table)
            int associated_str_tab_index = dynsym_header -> sh_link;
            Elf32_Shdr *associated_str_tab_header = &(shdr_start[associated_str_tab_index]);
            int associated_str_tab_offset = associated_str_tab_header -> sh_offset;
            char *strtab = (char*)(s -> map_start + associated_str_tab_offset);
            
            printf("Relocation section '%s' at offset %#x contains 8 entries:\n", header_name, rel_header -> sh_offset);
            printf(" %-12s%-11s%-9s%-12s%-12s\n", "Offset", "Info", "Type", "Sym.value", "Sym. name");
            for(int j = 0; j < number_of_rel; j++)
            {
                Elf32_Rel *current_rel = &(rel_start[j]);
                int offset = current_rel -> r_offset;
                int info = current_rel -> r_info;
                int symbol_index = ELF32_R_SYM(info);
                Elf32_Sym *current_symbol = &(dynsym_start[symbol_index]);
                int sym_index_in_strtab = current_symbol -> st_name;
                char *symbol_name = &(strtab[sym_index_in_strtab]);
