...
int main(int argc, char **argv)
{
    ...
    debug_sys(SYS_OPEN, isInDebugMode, "OPEN: ", curretDirectoryFileDescriptor);
    if ((count = system_call(GETDENTS, curretDirectoryFileDescriptor, dirBuf, MAX_DIRECTORY_DATA)) < 0)
    {
        system_call(SYS_EXIT, 0x55);
    }
    debug_sys(GETDENTS, isInDebugMode, "GETDENTS: ", count);
    i = 0;
    while (i < count)
    {
